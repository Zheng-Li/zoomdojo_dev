  ...
  <div class="content clearfix"<?php print $content_attributes; ?>>
    ...
    <?php if ($teaser): ?>
      <div>
        ...
        <?php endif; ?>
        <?php if ( !empty($node->field_resume_title[$lang][0]) ) : ?>
          <div>
            <a href="/young-professionals/resume/<?php print $node->nid; ?>">
              <?php
                $resumeTitle = field_view_value('node', $node, 'field_resume_title', $node->field_resume_title[$lang][0]);
                print render($resumeTitle);
              ?>
            </a>
          </div>
        <?php endif; ?>
    ...
    <?php else: ?>
      <div>
        ...
        <?php 
            ...
            $image = '';
            if ( !empty($node->field_type[$lang][0]) ) {
              $titlePrefix = $node->field_type[$lang][0]['taxonomy_term']->name . ' ';
              $image = $node->field_type[$lang][0]['taxonomy_term']->field_image;
            }
        ?>
            <!-- Share button begin -->
            <a href="<?php print $link; ?>" 
                class="btn btn-danger btn-share" 
                data-img="<?php print file_create_url($image[$lang][0]['uri']);?>"
